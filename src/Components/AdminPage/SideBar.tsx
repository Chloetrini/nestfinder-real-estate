...
// because i needed to make this responsive i changed the inline styling to tailwind, i hope yall don"t mind
const Sidebar = () => {
...
//  this fixed to the bottom is for the mobile screen so it remains on the bottom
    return (
            ...
            <div className="w-full flex lg:flex-col">
                ...
                <div className="flex flex-row lg:flex-col w-full lg:mt-[15px] lg:gap-6">
                    ...
                    <div className="flex flex-row lg:flex-col w-full justify-around lg:justify-start lg:gap-5 py-2 lg:py-0">
                        ...
                        <div 
                            onClick={() => navigate("/")} 
                            className="flex flex-col items-center w-full py-2 px-4 gap-1 cursor-pointer lg:hidden text-[#4F887B]"
                        >
                            <svg className="w-6 h-6 opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
                            <span className="font-lato text-[12px]">Home</span>
                        </div>

                        {/* Dashboard */}
                        <div 
                            onClick={handleClick} 
                            className={`flex flex-col lg:flex-row items-center lg:items-center w-full lg:w-[260px] py-2 lg:py-[9px] px-4 lg:px-4 gap-1 lg:gap-2 cursor-pointer transition-colors
                            ${isActive("/adminPage") ? "lg:bg-[#1A3C34] text-[#1A3C34] lg:text-white" : "text-[#4F887B] lg:bg-transparent"}`}
                        >
                            <img className={`w-6 h-6 ${isActive("/adminPage") ? "" : "opacity-70"}`} src={home} alt="home" />
                            <span className="font-lato text-[12px] lg:text-[16px]">Dashboard</span>
                        </div>
                        ...
                        >
                            <img className={`w-6 h-6 ${isActive("/adminPage/add-property") ? "" : "opacity-70"}`} src={circle} alt="add" />
                            <span className=" hidden md:block font-lato text-[12px] lg:text-[16px]">Add Property</span>
                            <span className="font-lato text-[12px] lg:text-[16px] block md:hidden">Add</span>
                        </div>
                        ...
                        >
                            <img className={`w-6 h-6 ${isActive("/adminPage/manage-property") ? "" : "opacity-70"}`} src={user} alt="manage" />
                            <span className=" hidden md:block font-lato text-[12px] lg:text-[16px]">Manage Property</span>
                            <span className="font-lato text-[12px] lg:text-[16px] block md:hidden">Manage </span>
                        </div>
                    </div>
                </div>
            </div>
           <div className="flex items-center px-4 lg:px-5 py-6 mt-auto"> 
            <div className="flex items-center gap-2">
                <img className="hidden lg:block" src={users} alt="user" />
                <button 
                onClick={() => navigate("/login")} 
               className="text-[#FF0000] font-lato text-[14px] lg:text-[16px] font-medium hover:underline">Logout </button>
            </div>
            </div>
        </div>
    )
