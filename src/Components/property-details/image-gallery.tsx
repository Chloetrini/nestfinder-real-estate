...
const Placeholder = ({ className }: { className: string }) => (
  <div className={`${className} bg-[#f3f4f6] dark:bg-gray-950 border-2 border-dashed border-[#BAB9B9] dark:border-gray-600 rounded-xl flex flex-col items-center justify-center gap-2`}>
  ...
  return ( 
    <div className="flex lg:flex-row flex-col gap-4">
        ...
        <div className="flex-1">
            {getImage(0) ? (
                <img 
                  src={optimizeImage(getImage(0), 1200)}
                  alt="Main Property"
                  fetchPriority="high"
                  decoding="async" 
        ...
        <div className="grid grid-cols-2 gap-4 flex-1">
            {[1, 2, 3, 4].map((index) => (
                getImage(index) ? (
                    <img 
                      key={index}
                      src={optimizeImage(getImage(index), 640)}
                      loading="lazy"
                      decoding="async" 
