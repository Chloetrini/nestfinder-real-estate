...
import search from "@/assets/icons/searchm.png"
...
const ManageProperties: React.FC = () => {
  useSeo({ title: 'Manage properties', noindex: true })
  const navigate = useNavigate()
  const { properties, deleteProperty, updateProperty } = useProperties()
  const [activepage, setActivePage] = useState<Filter>("All Properties")
 ...
 // ---- BACKEND ADDED: modal state for success and error messages ----
  ...
  return (
    <div className="flex flex-col bg-[#F3F4F6] dark:bg-gray-950 min-h-screen w-full pb-10">
      <nav className="h-[76px] w-full bg-white dark:bg-gray-900 px-4 md:px-10 flex items-center border-b border-[#BAB9B9] dark:border-gray-600 sticky top-0 z-20">
        <h1 className="font-['Lato'] font-bold text-[20px] md:text-[22px] text-[#023337] dark:text-gray-100">Manage Properties</h1>
      </nav>
      ...
      <div className="px-4 md:px-10 py-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div className="flex flex-col gap-2">
            <h2 className="font-bold text-[22px] text-[#023337] dark:text-gray-100 font-['Lato']">Manage Properties</h2>
            <p className="text-[14px] text-gray-600 dark:text-gray-300 font-['Lato']">Fill in the details below to list a new property</p>
          </div>
          <button 
            onClick={() => navigate("/adminPage/add-property")}
            className="w-full sm:w-auto bg-[#1A3C34] dark:bg-[#24574a] text-white px-6 py-3 rounded-[8px] font-bold text-[15px] hover:bg-[#264d43] dark:hover:bg-[#2d6a5a] transition-all transform hover:scale-105">
            Add New Property
          </button>
        </div>

    
        <div className="bg-white dark:bg-gray-900 rounded-[8px] shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col min-h-[600px]">
          
          <div className="flex flex-col xl:flex-row gap-4 p-4 justify-between border-b border-gray-50 dark:border-gray-700">
            <div className="flex overflow-x-auto no-scrollbar bg-[#D7FFF6] dark:bg-[#0f3b32] p-1 rounded-[8px] w-full xl:w-fit">
              {(["All Properties", "For Sale", "For Rent", "Featured", "Draft"] as const).map((tab) => (
                <button 
                  key={tab} 
                  onClick={() => { setActivePage(tab); setCurrentPage(1); }} 
                  className={`px-4 py-2 rounded-md text-[13px] md:text-[14px] font-medium whitespace-nowrap transition-all ${activepage === tab ? "bg-white dark:bg-gray-900 text-[#414242] dark:text-gray-100 shadow-sm" : "text-[#75928B]"}`}>
                  {tab}
                </button>
              ))}
            </div>
            <div className="relative w-full xl:w-[300px]">
              <input 
                ...
                className="w-full h-[44px] bg-[#EFF8F6] dark:bg-gray-800 rounded-[8px] px-4 pr-10 outline-none text-[14px] font-['Lato'] border border-transparent focus:border-[#1A3C34] dark:focus:border-[#3b8a76]"
              />
              <img className="absolute top-1/2 -translate-y-1/2 right-3 w-5 h-5 opacity-60 dark:invert" src={search} alt="search icon" />
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#B1FFED] dark:bg-[#0f3b32] h-[56px]">
                  <th className="px-6 text-[15px] font-bold text-[#023337] dark:text-gray-100">Property</th>
                  <th className="px-6 text-[15px] font-bold text-[#023337] dark:text-gray-100">Type</th>
                  <th className="px-6 text-[15px] font-bold text-[#023337] dark:text-gray-100">Location</th>
                  <th className="px-6 text-[15px] font-bold text-[#023337] dark:text-gray-100">Price</th>
                  <th className="px-6 text-[15px] font-bold text-[#023337] dark:text-gray-100">Status</th>
                  <th className="px-6 text-[15px] font-bold text-[#023337] dark:text-gray-100 text-center">Actions</th>
                </tr>
              </thead>
                ...
                {currentPropertyPagin.map((property) => (
                  <tr
                    ...
                    onClick={(e) => {
                      ...
                      navigate(`/adminPage/property/${property._id}`);
                    }}
                  ...
                  >
                    <td className="px-6 py-4 min-w-[250px]">
                      <div className="flex items-center gap-3">
                        {property.images && (
                          <img 
                            src={optimizeImage(Array.isArray(property.images) ? property.images[0] : property.images, 160)} loading="lazy" 
                            alt="" 
                    ...
                    <td className="px-6 py-4 text-[#403F3F] dark:text-gray-300 text-[14px] whitespace-nowrap">{property.propertyType}</td>
                    <td className="px-6 py-4 text-[#403F3F] dark:text-gray-300 text-[14px]">
                      <p className="truncate hover:whitespace-normal hover:overflow-visible max-w-[180px]">{property.location.fullAddress} </p>
                    </td>
                    <td className="px-6 py-4 font-bold text-[#023337] dark:text-gray-100 whitespace-nowrap">₦{property.price.toLocaleString()}</td>
                   ...
                   {/* ---- BACKEND UPDATED: show Draft status for draft properties ---- */}
                        ...
                        <span className="text-[#023337] dark:text-gray-100 font-medium text-[14px] whitespace-nowrap">{property.isDraft ? "Pending" : property.sale}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex justify-center items-center gap-3">
                        <button 
                          onClick={() => navigate(`/adminPage/edit-property/${property._id}`)} 
                          ...
                          {property.isDraft && (
                            ...
                            <span className="text-[#21C45D]">/</span>
                              <button
                                ...
                                className="text-[#1A3C34] dark:text-[#8fd3c0] font-normal text-[15px] hover:underline">
                                Publish
                              </button>
